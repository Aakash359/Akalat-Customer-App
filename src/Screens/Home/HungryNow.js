...
function HungryNow(props) {
  const hungryNowListResponse = useSelector(
    (state) => state.Home.hungryNowListResponse,
  )
  ...
  const addToCart = (item) => {
    ...
    if (
      cartRestroDetails &&
      cartRestroDetails?._id !== item?.restro_details?._id
    ) {
      ...
      <ImageBackground
                  ...
                  {inCart ? (
                    <View style={{flexDirection: 'row', alignItems: 'center'}}>
                      <Icon
                        ...
                        type="AntDesign"
                        name="minussquareo"
                        style={{fontSize: Scale(20), color: Colors.APPCOLOR}}
                      />
                      <Text
                        style={[styles.textStyle, {color: Colors.APPCOLOR}]}>
                        {' '}
                        {inCart?.qty}{' '}
                      </Text>
                      <Icon
        ...
        {cartProducts?.length ? (
          <View
            ...
            <TouchableOpacity
              onPress={() => props.navigation.navigate('Card', product_list)}
              style={{
